...
import { obtenerCiudades, obtenerDisponibilidadPorOficinas, getApiErrorMessage,
...
export function NuevoHorarioStep() {
  const { citaActiva, setNuevoSlot, setPaso } = useRescheduleStore();
  ...
  const handleContinuar = () => {
    ...
    setNuevoSlot(
      { date: scheduleSelected.date, hour: horaSeleccionada },
      slotOficinaId,
          ...
          {estado === "success" && oficinasDisponibles.length > 0 && (
            <>
              ...
              <div className="flex justify-between px-6 pb-6">
                <Button variant="outline" className="rounded-xl" onClick={() => setPaso(1)}>
                  Volver
                </Button>
                <Button
